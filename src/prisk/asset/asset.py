...
class Asset:
    ...
    def update_expected_damage(self) -> None:
        ...
        for flood_exposure in self.flood_exposure:
            ...
            expected_damage += (
                self.flood_damage_curve.loc[impact_depth].damage
    ...
    def flood(self, depth: float, time: pd.Timestamp):
        ...
        if self.rebuild_strategy == "build_back_better" and damage > 0:
            ...
            raise NotImplementedError(
                f"The rebuild strategy '{self.rebuild_strategy}' is not implemented."
            )

        self.business_disruption_path[year] += (
            self.revenue_path[year] * production / 365
        )
        ...
        self.business_disruption_path[year] -= self.cost_path[year] * min(
            0.5, production / (365 * 2)
        )
